...
New-GPLink -Name "EnableWinRM" -Target "ou=IT Computers,ou=IT,dc=miacsdomain,dc=com"

#add the security filtering, uncomment if not needed
Set-GPPermission -Name "EnableWinRM" -TargetName "Win-10" -TargetType Computer -PermissionLevel GPOApply

timeout /t 10 /nobreak

#set remote computers to start WinRM service automatically (needs restart)
Set-Service -Name "WinRM" -StartupType "Automatic" -ComputerName Win-10
